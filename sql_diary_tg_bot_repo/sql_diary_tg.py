...
class Database:
    ...
    def get_entries_by_date(self, tg_id, days_back=14):
        try:
            sql = """
                    SELECT date(timestamp) as entry_date, COUNT(*)
                    FROM entries
                    WHERE tg_id = ?
            ...
            conn = self.conn()
            cur = conn.cursor()
            cur.execute(sql, (tg_id, f"-{days_back} days"))
            result = cur.fetchall()
            conn.close()
            return result
        except Error as e:
