...
3. Run the bot using:
   ...
   ```sh
   python diary_bot_v4.py
   ```

## Docker

You can build a container image with all dependencies installed.

1. Create `config.ini` next to the Dockerfile:
...
2. Build the image:
   ...
   ```sh
   docker build -t sql-diary-bot .
   ```

3. Run the bot (the database will be stored in the container by default):
...
To persist the database between restarts, mount a volume and set the database path:

```sh
docker run --rm -it \
  -v "$(pwd)/config.ini:/app/config.ini:ro" \
  -v "$(pwd)/data:/app/data" \
  -e SQL_DIARY_DB_PATH=/app/data/sql_diary.db \
  sql-diary-bot
```
...
- **/stats** - show mood and activity charts (after password verification).
