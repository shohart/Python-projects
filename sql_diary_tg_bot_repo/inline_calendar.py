...
class InlineCalendar:
    ...
    CALLBACK_WRONG_CHOICE = BASE_CALLBACK.new(
        action=Actions.WRONG_CHOICE.name, data="-"
    )
    CALLBACK_PREVIOUS_MONTH = BASE_CALLBACK.new(
        action=Actions.PREVIOUS_MONTH.name, data="-"
    )
    ...
    def _create_bottom(self, chat_id: int) -> List[types.InlineKeyboardButton]:
        ...
        if user_info.current_date > user_info.min_date:
            result.append(
                types.InlineKeyboardButton(
                    "<", callback_data=InlineCalendar.CALLBACK_PREVIOUS_MONTH
                )
            )
        else:
            result.append(
        ...
        if user_info.current_date < user_info.max_date:
            result.append(
                types.InlineKeyboardButton(
                    ">", callback_data=InlineCalendar.CALLBACK_NEXT_MONTH
                )
            )
        else:
            result.append(
    ...
    def init(
        ...
        :param month_names: 12-element list for month names.
            If none, then English names will be used
        :param days_names: 7-element list fo2r month names.
            If none, then English names will be used
        """
        if chat_id is None:
        ...
        self._set_user_info(
            chat_id,
            InlineCalendarData(
                chat_id, min_date, max_date, base_date, month_names, days_names
            ),
        )
    ...
    def get_keyboard(self, chat_id: Optional[int] = None):
        ...
        kb.row(
            *self._create_header(chat_id)
        )  # create header with month name and year aka "Aug 2019"
        ...
        for i in range(1, mrange[1] + 1):  # adding active days
            curr_date = datetime.date(
                day=i,
                month=user_info.current_date.month,
                year=user_info.current_date.year,
            )
            ...
            if curr_date < user_info.min_date:
                ...
                rows[-1].append(
                    types.InlineKeyboardButton(
                        text=str(i),
                        callback_data=InlineCalendar.BASE_CALLBACK.new(
    ...
    def handle_callback(self, chat_id: int, callback_data: Union[Dict[str, str], str]):
        ...
        action, data = callback_data.get("action", None), callback_data.get(
            "data", None
        )
        ...
        user_info = self._get_user_info(chat_id)
        if (
            callback_data["action"] == Actions.PREVIOUS_MONTH.name
            and user_info.current_date > user_info.min_date
        ):
            self._dec_month(chat_id)

        if (
            callback_data["action"] == Actions.NEXT_MONTH.name
            and user_info.current_date < user_info.max_date
        ):
